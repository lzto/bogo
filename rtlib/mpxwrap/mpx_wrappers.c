...
__mpx_wrapper_realloc (void *ptr, size_t n)
{
  ...
  if (!new_ptr)
  {
    return __bnd_null_ptr_bounds (new_ptr);
  }
  /*
   * move maximum bound possible.
   * new memory region may be allocated,
   * if that happens try to move bounds also
   */
  #if 0
  if (new_ptr!=ptr)
  {
      move_bounds(new_ptr, ptr, n);
  }
  #endif
  return __bnd_set_ptr_bounds (new_ptr, n);
}
